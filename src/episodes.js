...
export async function loadLocalEpisodes() {
  const response = await fetch("./data/dragon-ball.json");
  if (!response.ok) throw new Error(`Lokalna baza odcinków: ${response.status}`);
  const database = await response.json();
  if (!Array.isArray(database.episodes) || database.episodes.length !== 153) {
    throw new Error("Lokalna baza musi zawierać 153 odcinki");
  }
  dragonBallTitles = database.episodes.map((episode) => episode.title);
  return { count: dragonBallTitles.length, importedAt: database.importedAt };
}
